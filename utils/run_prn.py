...
def main():
    ...
    assert job_gubun in ["prnw", "prna", "prnbase"]
    ...
    base_conf["dataset_config_name"] = ds_nm
    base_conf["output_dir"] = env_config["model_output_dir"] + job_cd
    base_conf["cache_dir"] = env_config["cache_dir"]
    base_conf["model_name_or_path"] = model_path_format.format(model_id=ds_nm)
    ...
    send_example_telemetry("run_prn", model_args, data_args)
    ...
    if job_gubun in ["prnw", "prna", "prnbase"]:
        df_local_result = pd.read_csv(df_local_result_path, sep="\t")
        if job_gubun in ["prnw", "prna"]:
            # col = "threshold_w" if "threshold_w" in df_local_result.columns else
            if prn_prop == "top":
