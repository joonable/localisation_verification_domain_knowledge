    ...
    if job_gubun in ["pftw", "pfta", "pftbase"]:
        ...
        if job_gubun in ["pftw", "pfta"]:
            df_pft_components = df_local_result \
                .loc[(df_local_result[f"threshold"] <= pft_prop) | (~(df_local_result["is_investigated"]))]

        elif job_gubun == "pftbase":
            df_pft_components = df_local_result.loc[
                (df_local_result["layer"] == -1)
                | (df_local_result["layer"] >= int(12 * (1 - pft_prop / 100)))
                | ~(df_local_result["is_investigated"])
                ]

    assert df_pft_components is not None
